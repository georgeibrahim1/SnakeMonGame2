...
void Card11::GetCopy(Grid* pGrid)
{
	Card* c;
	c = new Card11(this->GetPosition());
	c->SetCardNumber(11);
...
void Card11::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	Output* pOut = pGrid->GetOutput();
	Input* pIn = pGrid->GetInput();
	if (!pGrid->getwasexecuted(11))// False means the card is not owned yet
	{
		int Flag = 0;
		...
		{
			pGrid->setwasexecuted(11, true);//To indicate this card is owned by a player
			pPlayer->SetWallet((pPlayer->GetWallet()) - cardprice);
			pOut->PrintMessage("Player " + to_string(pPlayer->GetPlayerNum()) + " is the owner of Card 11");
			this_thread::sleep_for(chrono::seconds(2));
			ownernum = (pPlayer->GetPlayerNum());// A static variable holding the player number of owner
		}

	}
	else if (pGrid->getwasexecuted(11))// True means the card is owned 
	{
