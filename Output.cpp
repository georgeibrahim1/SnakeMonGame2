...
Output::Output()
{
	...
	// Change the title
	pWind->ChangeTitle("Snakes & Ladders Game");
...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	/// Calculate the Width & Height of the integer "cardNum" if written using the Current Font
	...
	// Draw the Integer the representing the "cardNum" in the location (x,y)
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_LOAD_GRID] = "images\\cloud-computing.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\dice.jpg";
	MenuItemImages[ITM_NEW_GAME] = "images\\controller.jpg";
	MenuItemImages[ITM_EXIT_PM] = "images\\Menu_Exit.jpg";
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	/// Validate the playerNum, if not valid return
	...
	///Draw the player circle in center(x,y) and filled with the playerColor passed to the function
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	///Validate the Cell Position (Must be Vertical Cells AND toCell is above fromCell, otherwise, Do NOT draw)
	if (!((fromCell.HCell() == toCell.HCell()) && (toCell.GetCellNum() > fromCell.GetCellNum())))
	{
		return;
	}
	...
	///Set pen color and width using the appropriate parameters of UI_Info object (UI)
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);


	///Draw The First Vertical Line (The Left Line) in the appropriate coordinates
	...
	/// Set pen color and width using the appropriate variables of UI_Info object (UI)
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);



	///Draw The Second Vertical Line (The Right Line) in the appropriate coordinates
	...
	/// Draw the cross horizontal lines of the ladder using the appropriate coordinates

	int YLine = fromStartY;
	for (int i = 0; i < ((fromCell.VCell() - toCell.VCell())); i++)
	{
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		pWind->DrawLine(x12, YLine, x34, YLine);
		YLine -= UI.CellHeight;
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	/// Set pen color and width from the appropriate variables of the UI_Info object (UI)
	pWind->SetPen(UI.SnakeColor, UI.SnakelineWidth);


	/// Draw the Line representing the Snake BOdy
	pWind->DrawLine(x12, y1, x12, y2);
	...
	///Set the coordinates of the 4 points of the Polygon
	//Check the snakes drawn in the project document and draw it the same way
	int centerY = fromStartY + UI.LadderYOffset;
	int centerX = x12;

	int Xpluschange = centerX + xChange / 2;
	int Ypluschange = centerY + yChange / 2;
	int X_change = centerX - xChange / 2;
	int Y_change = centerY - yChange / 2;

	int XAxis[4] = { X_change  , centerX  ,Xpluschange ,centerX };
	int YAxis[4] = { centerY ,Y_change , centerY,Ypluschange };

	/// Draw the Polygon (diamond) representing the Snake's Head
	//  Check the snakes drawn in the project document and draw it the same way
	pWind->DrawPolygon(XAxis, YAxis, 4);

	// ---- 3 - Draw Eyes of the snakes ---- (Addition)


	pWind->SetPen(RED, 1);
	pWind->SetBrush(RED);

	int lEyeX = (centerX - xChange / 3) + 1;
	int lEyeY = (centerY - yChange / 3) + 2;
	pWind->DrawCircle(lEyeX, lEyeY, 1);


	int rEyeX = (centerX + xChange / 3) + 1;
	int rEyeY = lEyeY;
	pWind->DrawCircle(rEyeX, rEyeY, 1);

	// ---- 3 - Draw Tongue of the snakes ---- (Addition)


	pWind->SetPen(RED, 1);


	int tongueStartX = centerX;
	int tongueStartY = Y_change - 2;
	int tongueEndX = centerX;
	int tongueEndY = tongueStartY - 6;


	pWind->DrawLine(tongueStartX, tongueStartY, tongueEndX, tongueEndY);
